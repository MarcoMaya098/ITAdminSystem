...
$ingresos_array = array();
$numRegistros = $ingreso->numRegistros($id);
$arr = array("Pesos", "Dolares"); 
/*******************
...
if (isset($_POST["enviar"])) {
    ...
    if(!validaFecha($fecha)) $msg[] = "1El campo de 'fecha' es requerido.";
	if(!validaRequerido($empresa)) $msg[] = "1El campo de 'empresa' es requerido.";
	if(($moneda=="0")) $msg[] = "1Debes seleccionar un tipo de moneda.";
	...
	if (count($msg)==0) {							
		...
		$ingreso->altaingreso($idingreso, $id, $fecha, $cantidad, $iva, $empresa, $concepto, $moneda, $tipoCambio);		
		}
		else{
			$m = "A";			
		}	
...
if($m=="D"){
	$idingreso = $_GET["id"];
	$ingreso->borraringreso($idingreso);	
	$m="S";
}
//Consulta o baja previa del registro
if($m=="C" || $m=="B"){
	$idingreso = $_GET["id"];
	$data= $ingreso->leeRegistro($idingreso);
	$idingreso=$data[0]["id"];
	$fecha=$data[0]["fecha"];
	...
	$ingresos_array = $ingreso->leeingresosUsuario($id,$inicio,$TAMANO_PAGINA);
}
?>
<!DOCTYPE html>
<html>
	...
	<script>
		window.onload = function(){
			...
			document.getElementById("alta").onclick = function(){
				window.open("ingresos.php?m=A", "_self");}
			<?php  }?>

			<?php if ($m=="C" || $m=="A"){?>
			document.getElementById("regresar").onclick = function(){
				window.open("ingresos.php", "_self");}
			...
			document.getElementById("simon").onclick = function(){
				var idingreso = <?php print $idingreso; ?>;
				window.open("ingresos.php?m=D&id="+idingreso, "_self");}
			<?php  }?>
		}
		function cambiaPagina(p) {
			window.open("ingresos.php?p="+p,"_self");
		}
		function toggle(o) {
			var el=document.getElementById("mostrar");
			if (o.value=="Dolares") el.style.display="block";
			if (o.value=="Pesos") el.style.display="none";
			if (o.value=="0") el.style.display="none";
		}
	</script>
	<style>
...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark mb-3">
	<div class="container-fluid">
		...
		<ul class="navbar-nav mr-auto mt-2 mt-lg-0">
			...
			<li class="nav-item ">
				<a href="ingresos.php" class="nav-link active">Ingresos</a>
			</li>
			<li class="nav-item">
	...
	<div class="container-fluid text-center">
		<div class="row content">
				...
				<?php if($m=="C" || $m=="A" || $m=="B") {
					...
					<div class="form-group text-start mb-3" >
						...
						<div class="form-group text-start mb-3" >
							...
							placeholder="Escribe el nombre de la empresa" value="<?php print $empresa; ?>" <?php print ($m=='B')?'disabled':""; ?>/>
						</div>
						<div class="form-group text-start mb-3">
						...
						<div class="form-group text-start mb-3">
							...
							placeholder="Digita el iva en dolares o pesos" value="<?php print number_format($iva,2); ?>" <?php print ($m=='B')?'disabled':""; ?>/>
						</div>
						<input type="hidden" name="idingreso" id="idingreso" value="<?php print $idingreso; ?>">
						<?php if ($m=="C" || $m=="A") { ?>
						<div class="form-group text-start mb-3">
			...
			<?php                      
				...
				for($i=0;$i< count($ingresos_array) ;$i++){
				print "<tr>";
				//print "<td>".$ingresos_array[$i]["id"]."</td>";
				print "<td class='text-start'>".$ingresos_array[$i]["concepto"]."</td>";
				print "<td>".$ingresos_array[$i]["fecha"]."</td>";
				print "<td class='text-start'>".number_format($ingresos_array[$i]["cantidad"],2)."</td>";
				print "<td class='text-start'>".number_format($ingresos_array[$i]["iva"],2)."</td>";
				print "<td class='text'>".$ingresos_array[$i]["empresa"]."</td>";
				print "<td><a class='btn btn-primary' href='ingresos.php?m=C&id=".$ingresos_array[$i]["id"]."'>Modificar</a></td>";
				print "<td><a class='btn btn-danger' href='ingresos.php?m=B&id=".$ingresos_array[$i]["id"]."'>Borrar</a></td>";
				print "</tr>";	
				}				
				print "</table>";
			...
			<?php if($m=="S"){?>
					...
					<input type="button" name="alta" id="alta" class="btn btn-info" value="Dar de alta un ingreso" role="button"/>
				<?php }?>
			</div>
			<div class="col-sm-2 sidevar">
			</div>
