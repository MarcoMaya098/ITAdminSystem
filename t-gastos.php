...
$id = $data["id"];
$totalgasto = new Total();
$gastoTotal_array = array();
...
if (isset($_POST["year"])) {
    ...
    if ($year=="0") {
        ...
        if ($totalgasto->obtnerTotalGasto($year, $mes1, $mes2,$id)){
            $gastoTotal_array = $totalgasto->obtnerTotalGasto($year, $mes1, $mes2, $id);
			$GastoPagado8 = ($gastoTotal_array[0]["GastoPagado8"]);      
            $GastoPagado16 = ($gastoTotal_array[0]["GastoPagado16"]); 
			$iva8 = ($gastoTotal_array[0]["iva8"]);      
            $iva16 = ($gastoTotal_array[0]["iva16"]); 
			$SumaGastos = ($gastoTotal_array[0]["SumaGastos"]);      
            $IvaRetenido = ($gastoTotal_array[0]["IvaRetenido"]); 
			if($GastoPagado8==0 && $GastoPagado16==0 && $SumaGastos==0 && $IvaRetenido==0)
...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark mb-3">
	...
	<ul class="navbar-nav mr-auto mt-2 mt-lg-0">
		...
		<li class="nav-item">
			<a href="t-gastos.php" class="nav-link active">S-Gastos</a>
		</li>
		<li class="nav-item">
		...
		<div class="row content">
			...
			<div class="col-sm-8 text-center">
				<?php require "php/mensajes.php"; ?>
					<h2>Suma de Gastos</h2>
					<form action="t-gastos.php" method="POST">
						...
						<div class="form-group text-start mb-3">
							...
							<input type="submit" name="total" id="total" class="btn btn-success" value="Ver gastos"/>
						</div>												
					</form>	</br>			
			<?php    									
				print "<table class='table table-striped' width='100%'>";
				print "<tr>";
				print "<th>Gasto pagado 8%</th>";
                print "<th>Gasto pagado 16%</th>";
				print "<th>IVA 8%</th>";
                print "<th>IVA 16%</th>";
                print "<th>Suma de gastos</th>";
				print "<th>IVA retenido</th>";

				print "</tr>";
					print "<tr>";
					print "<td>".number_format($GastoPagado8,2)."</td>";
					print "<td>".number_format($GastoPagado16,2)."</td>";
					print "<td>".number_format($iva8,2)."</td>";
					print "<td>".number_format($iva16,2)."</td>";
                    print "<td>".number_format($SumaGastos,2)."</td>";
					print "<td>".number_format($IvaRetenido,2)."</td>";
					print "</tr>";
				print "</table>";															
            ?>
			</div>
